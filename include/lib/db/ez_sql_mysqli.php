	...
	class ezSQL_mysqli extends ezSQLcore
	{
		...
		*/
		...
		function connect($dbuser='', $dbpassword='', $dbhost='localhost', $dbport=false)
		{
			...
			{
              if(defined('MYSQL_SSL_CERTIFICATE_PATH') && !empty(MYSQL_SSL_CERTIFICATE_PATH)){
                $this->dbh = mysqli_init();
                mysqli_ssl_set($this->dbh, NULL, NULL, MYSQL_SSL_CERTIFICATE_PATH, NULL, NULL);
                mysqli_real_connect($this->dbh, $dbhost, $dbuser, $dbpassword, '', $dbport, MYSQLI_CLIENT_SSL, MYSQLI_CLIENT_SSL_DONT_VERIFY_SERVER_CERT);
              }
              else{
				...
				if( $this->dbh->connect_errno )
				{
					...
					$this->register_error('DB connection error');
		...
		*/
        ...
        if(defined('TRACKER_DEBUG') && TRACKER_DEBUG){
          $str .= "\n".'<br />Sql Query: '.$query."\n";
        }
		...
		*/
		...
		function disconnect()
		{
      @$thread = $this->dbh->thread_id;
      @$this->dbh->kill($thread);
			@$this->dbh->close();
		}
