<?php
	...
	*/
	...
	class ezSQLcore
	{
		...
		var $cache_queries    = true;
		var $cache_inserts    = false;
		var $use_disk_cache   = false;
		var $cache_timeout    = 0.15; // hours
		var $timers           = array();
		...
		*/
		...
		function cache_handler($mode)
		{
			$this->use_disk_cache = $mode;
		}
    ...
    function update($table, $data, $where){
      ...
      $this->query("UPDATE `$table` SET ".implode(',', $update)." WHERE ".implode(' AND ', $wh));
    }
    ...
    function delete($table, $where){
      ...
      $this->query("DELETE FROM `$table` WHERE ".implode(' AND ', $wh));
    }
