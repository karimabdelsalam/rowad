...
- [ ] تأكد أن مجلد `uploads/` قابل للكتابة (صلاحية 755) لرفع المرفقات.
- [ ] لشاشة غرفة الانتظار: افتح `موقعك/clinic/queue.php?screen=1` على تابلت أو
      شاشة، ثم اضبطها على وضع ملء الشاشة.
...
| فشل رفع المرفقات | تأكد أن `uploads/` موجود وقابل للكتابة، وأن `upload_max_filesize` في PHP أكبر من الحد المضبوط |
