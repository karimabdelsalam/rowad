      ...
      <table class="table table-condensed table-striped table-primary table-vertical-center checkboxs">
        <thead>
          <tr>
            ...
            <th>{"الاسم"|gettext}</th>
            <th class="center">{"فئة العقار"|gettext}</th>
            <th class="center">{"عدد الحقول"|gettext}</th>
        ...
        <tbody>
          {foreach from=$results item=result}
            <tr class="selectable">
              ...
              <td>
                <strong><a href="{$CPURL}/custom_fields/index?catid={$result.id}">{$result.title}</a></strong>
              </td>
              <td class="center"><span class="label label-primary">{$result.buildCat}</span></td>
              <td class="center"><span class="label label-warning">{$result.fieldsCount}</span></td>
