<div class="filter-bar">
  <form action="" method="get" class="form-horizontal innerAll">
    <div class="form-group">
      <label class="col-md-2">{"اسم المستأجر"|gettext}</label>
      <div class="col-md-3">
        <select class="buildrenterAjaxSearch noSelect2" name="renterid" style="width:100%" required>
          {if $smarty.get.renterid}<option value="{$smarty.get.renterid}">{$from.fname} {$from.fathname} {$from.lname} {$from.famname}</option>{/if}
        </select>
      </div>
    </div>
    <div class="form-group">
    ...
    <div class="widget-body">
      <table class="table table-condensed table-striped table-primary table-vertical-center checkboxs">
        ...
        <tbody>
          ...
          {foreachelse}
            <tr class="warning"><td class="center" colspan="20">{"لا يوجد اي بيانات متعلقة بالمستأجر في تلك الفترة"|gettext}</td></tr>
          {/foreach}
          {if !empty($results)}
            <tr>
