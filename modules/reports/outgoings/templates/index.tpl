<div class="filter-bar">
  <form action="" method="get" class="form-horizontal innerAll">
    <div class="form-group">
      <label class="col-md-2">{"جهة الصرف"|gettext}</label>
      <div class="col-md-3">
        <select name="from_type" class="form-control">
          <option value="">{"اختر جهة الصرف"|gettext}</option>
          {foreach $methods as $method}
          <option value="{$method.name}" {if $smarty.get.from_type eq $method.name}selected="selected"{/if}>{$method.title}</option>
          {/foreach}
        </select>
      </div>
    </div>
    <div class="form-group">
      <label class="col-md-2">{"تاريخ الصرف"|gettext}</label>
      <div class="col-md-3">
      ...
      <table class="table table-condensed table-striped table-primary table-vertical-center checkboxs">
        <thead>
          <tr>
            ...
            <th class="center" style="width:80px">{"رقم السند"|gettext}</th>
            <th class="center">{"جهة الصرف"|gettext}</th>
            <th class="center">{"المبلغ"|gettext}</th>
            <th class="center">{"نوع الصرف"|gettext}</th>
            <th class="center">{"الطريقة"|gettext}</th>
            <th class="center">{"تاريخ الصرف"|gettext}</th>
        ...
        <tbody>
          {foreach from=$results item=result}
            <tr class="selectable">
              ...
              <td class="center"><span class="label label-primary">{$result.type}</span></td>
              <td class="center"><span class="label label-warning">{$result.pay_method|coregate:"payMethodTitle"}</span></td>
          ...
          {/foreach}
          {if $results[0].totals.totalAmounts gt 0}
          <thead>
            <tr>
              <th class="uniformjs"></th>
              <th class="center">{$results[0].totals.totalCounts}</th>
              <th colspan="2">{$results[0].totals.totalAmounts|number_format:2} {$config.currency}</th>
              <th colspan="10"></th>
            </tr>
          <thead>
          {/if}
