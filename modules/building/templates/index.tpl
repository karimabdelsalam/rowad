<div class="filter-bar">
  <form action="" method="get" class="form-horizontal innerAll">
    ...
    <div class="form-group hide2">
      <label class="col-md-2">{"المالك"|gettext}</label>
      <div class="col-md-4">
        <select name="ownerid" class="ownerAjaxSelcSearch noSelect2" style="width:100%">
          <option value="{$owner.id}">#{$owner.id} {$owner.fullname}</option>
        </select>
      </div>
    </div>
    <div class="form-group hide2">
                    ...
                    <div class="col-md-3">
                      <table>
                        <tr>
                          ...
                          {if !empty($result.nextpayment)}
                          <td>{$result.nextpayment.amount} {$config.currency} <button href="{$CPURL}/pays_pending/partpay/?id={$result.nextpayment.id}" title="{"تحصيل الدفعة القادمة"|gettext}" data-toggle="tooltip" class="btn btn-default quick-form btn-xs"><i class="fa fa-money"></i></button></td>
                          {else}
                          <td>{"خالص"|gettext}</td>
                          {/if}
                        </tr>
                      ...
                      <table>
                        <tr>
                          ...
                          {if $result.type eq "rent"}
                              <a href="{$CPURL}/buyer/edit/?id={$result.contract_info.buyer.profile[0].id}" class="quick-form">
                          ...
                          {else}
                              ...
                              {section name=op loop=count($result.contract_info.buyer.id)}
                                  <p>
                                    <a href="{$CPURL}/buyer/edit/?id={$result.contract_info.buyer.profile[op].id}" class="quick-form">
