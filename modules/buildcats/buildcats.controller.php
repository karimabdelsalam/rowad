...
class buildcats extends buildcats_model
{
  ...
  public function add()
  {
    if($_POST){
      ...
      $building_catsid = $this->addrecord($data);
      $this->registerLog($building_catsid);
      if($_POST['quick_form']){
        $this->showmsg(json_encode(array('id' => $building_catsid, 'title' => $_POST['title'], 'element' => $_POST['elementID'])), 'AutoAdder');
      }
      $this->showmsg(CPURL . '/' . Module . '/', 1);
    } else{
      $this->Output('edit');
