<div class="filter-bar">
  <form action="" method="get" class="form-horizontal innerAll">
    <div class="form-group">
      <label class="col-md-2">{"اسم البنك"|gettext}</label>
      <div class="col-md-3">
        <input type="text" name="bank" value="{$smarty.get.bank}" class="form-control" />
      </div>
    </div>
      ...
      <table class="table table-condensed table-striped table-primary table-vertical-center checkboxs">
        <thead>
          <tr>
            ...
            <th class="center">{"البنك"|gettext}</th>
            <th class="center">{"رقم الحساب"|gettext}</th>
            <th class="center">{"رصيد الحساب"|gettext}</th>
        ...
        <tbody>
          {foreach from=$results item=result}
            <tr class="selectable">
              ...
              <td>
                <strong>{if $result.main eq 1}<i class="fa fa-asterisk"></i> {/if}{$result.fullname}</strong>
              </td>
              <td class="center">{$result.bank}</td>
              <td class="center"><span class="label label-primary">{$result.accno}</span></td>
              <td class="center"><span class="label label-success">{$result.balance} {$config.currency}</span></td>
              <td class="text-left center no-print">
                {include file="core/templates/module_actions.tpl"}
              </td>
            </tr>
          {foreachelse}
            <tr class="warning"><td class="center" colspan="20">{"لا يوجد بيانات متاحة للعرض"|gettext}</td></tr>
            {/foreach}
        </tbody>
      </table>
