...
class borders extends borders_model
{
  ...
  {
    if($_POST){
      foreach($_POST['title'] as $key => $name){
        if(empty($name) || empty($_POST['attrs'][$key])) continue;
        $data = array();
        $data['title'] = $name;
        $data['attrs'] = $_POST['attrs'][$key];
