    ...
    <div class="form-group">
      ...
      <div class="col-md-3">
        <select name="typeid" class="form-control">
          <option value="">{"اختر نوع المستحقات"|gettext}</option>
...
{if $smarty.get.quick_form eq 0}
  <div class="navbar-blocks">
    <a href="{$CPURL}/{$cumodule.name}/processed/?{$params}" class="btn btn-success"><i class="fa fa-4x fa-calendar-check-o"></i> <p class="text-larger marginTB">{"منتهية"|gettext}</p></a>
    <a href="{$CPURL}/{$cumodule.name}/pending/?{$params}" class="btn btn-primary"><i class="fa fa-4x fa-calendar"></i><p class="text-larger marginTB">{"غير منتهية"|gettext}</p></a>
  </div>
{/if}
