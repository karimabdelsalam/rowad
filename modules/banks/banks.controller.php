...
class banks extends banks_model
{
  ...
  public function add()
  {
    if($_POST){
      ...
      $bankid = $this->addrecord($data);
      $this->registerLog($bankid);
      if($_POST['quick_form']){
        $this->showmsg(json_encode(array('id' => $bankid, 'title' => $_POST['title'], 'element' => $_POST['elementID'])), 'AutoAdder');
      }
      $this->showmsg(CPURL . '/' . Module . '/', 1);
    } else{
      $this->Output('edit');
