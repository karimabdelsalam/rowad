    ...
    <tr class="selectable">
      ...
      <td class="center">
          {if !empty($result.commission)}
            <span class="label label-warning" data-toggle="tooltip" data-placement="top" title="{"عمولة المكتب يتحمله المالك"|gettext}">{$result.commission} {$config.currency}</span>
          {/if}
          {if !empty($result.commission2)}
            <span class="label label-default" data-toggle="tooltip" data-placement="top" title="{"عمولة المكتب يتحمله العميل"|gettext}">{$result.commission2} {$config.currency}</span>
          {/if}
      </td>
      {if empty($shortcut)}
