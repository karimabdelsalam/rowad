          ...
          {foreach from=$results item=result}
            <tr class="selectable">
              ...
              <td class="center">
                ...
                <span class="label label-info marginTB">{$result.owner.name[op]}</span><br />
                {/section}
              </td>
              {if empty($build)}
              ...
              <td class="center"><span class="label label-primary">{$result.yearlyrent} {$config.currency}</span></td>
              <td class="center">
                {if $result.ended eq 0}
                  <span class="label label-success">{"نشط"|gettext}</span>
                {else}
                  <span class="label label-default">{"منتهي"|gettext}</span>
                {/if}
              </td>
